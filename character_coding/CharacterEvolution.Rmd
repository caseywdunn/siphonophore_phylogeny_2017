...
###Depth barplot annotations figure
```{r bathy_barplot, echo=F, message=F, warning=F}
#Depth_data_VARS
d <- lapply(runif(30, 0, 30), rnorm, n=100) #random data to be replaced
ylim <- range(unlist(d))
...
p <- ggtree(dpruned_tree) + geom_tiplab()
...
```{r pPCA_depth, echo=F, fig.height=10, fig.width=19, message=F, warning=F}
