...
###SIMMAP BRACTS:
```{r bracts, echo=F, message=F, warning=F}
#Simmap bracts
plotTree(ultratree, lwd = 4)
#plot(bract_sim_plot[2])
colors = c("black", "red")
names(colors) = c("Absent","Present")
bracts_sim %>% plotSimmap(lwd = 4, add = T, colors=colors) -> bract_sim_plot
nodelabels(pie=(describe.simmap(bracts_sim, plot=F)$ace) ,piecol=colors,cex=0.35)
add.simmap.legend(colors = colors, x=0.6*par()$usr[1],y=0.3*par()$usr[4],prompt=FALSE)
densityMap(bracts_sim)
```
...
Posterior probabilities of states (1 Present, 0 Absent).It is not clear whether or not the stem group of siphonophores had a pneumatophore.
...
```{r ggtree_sex, echo=F, message=F, warning=F, include=F}
exp_tree = ultratree
...
```{r ggtree_tentillq, echo=F, message=F, warning=F, include=F}
exp_tree = ultratree
exp_tree$edge.length = 10*ultratree$edge.length
...
```{r ggtree_palpon, echo=F, message=F, warning=F, include=F}
exp_tree = ultratree
exp_tree$edge.length = 10*ultratree$edge.length
...
######VARS data barplot code
```{r vars_plot, echo=F, message=F, warning=F}
#Get Raw Data
...
raw_data$Species[which(raw_data$Species == "Cordagalma")] = "Cordagalma ordinatum"
...
#Plots for FIGURE
ggplot(parched_data, aes(x = Species, y=-Depth)) + geom_boxplot() + theme_classic()
ggplot(parched_data, aes(x = Species, y=-Depth)) + geom_violin() + theme_classic()
